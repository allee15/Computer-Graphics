...
void UseMouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:			//	CLICK stanga => observatorul se muta in fata poligonului;
		if (state == GLUT_DOWN) {
			mode = 1;
			Initialize();
		}				//	Se reinitializeaza contextul OpenGL => redesenarea scenei;
		break;
...
GLuint fata = GL_FRONT;
GLuint spate = GL_BACK;
...
void RenderFunction(void)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, 3);							//	Schimbarea variabilei din shader cu valoarea codCol;
...
//lutCloseFunc(Cleanup);             //  Eliberarea resurselor alocate de program;
